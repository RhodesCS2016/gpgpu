...
//grid on 2D block 1D grid one loop
...
//grid on 2D block 1D grid 16 loop
__global__ void sumMatrixOnGPU1D(float *A, float *B, float *C, int NX, int NY)
{
    unsigned int idx = ((blockIdx.x * blockDim.x * blockDim.y) + (threadIdx.y * blockDim.x) + threadIdx.x);

    unsigned int max = (NX*NY+ loop -1)/loop;
...
int main(int argc, char **argv)
{
   ...
   // setup kernel launch parameters
    int dimx = 32; // default block size if no runtime parameters given
    int dimy = 32;
    ...
    dim3 grid((((nx + block.x - 1)/block.x)*((ny + block.y - 1) / block.y) + loop - 1)/loop,1);
